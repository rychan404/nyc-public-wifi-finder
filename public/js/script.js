...
window.addEventListener('scroll', function() {
  let scrollT = this.document.documentElement.scrollTop || this.document.body.scrollTop;
  let scrollH = this.document.documentElement.scrollHeight || this.document.body.scrollHeight;

  let percent = scrollT / (scrollH - this.document.documentElement.clientHeight) * 10;
  const foreground = document.querySelectorAll('.foreground');
  let divide = 1;
  
  foreground.forEach(element => {
    element.style.width = (100 + percent / divide) + '%';
    divide++;
  });
});
